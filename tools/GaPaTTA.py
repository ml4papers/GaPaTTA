...
from mmseg.apis import multi_gpu_test, single_gpu_test, single_gpu_gapatta
...
def parse_args():
    ...
    parser.add_argument('--consistency_weight', type=float, default=0.4, help='跨阶段一致性权重')
    ...
    parser.add_argument('--num_domain_epochs', type=int, default=3)
...
def main():
    ...
    print("consistency_weight", args.consistency_weight)
    print("entropy_thresh", args.entropy_thresh)
    print("num_itr", args.num_itr)
    ...
    #         outputs = single_gpu_gapatta(args, model, data_loader, args.show, args.show_dir,
    ...
    for itr in range(args.num_itr):
        print(f"\n================ Iteration {itr + 1}/{args.num_itr} (All Domains) ================")
        wandb.log({"iteration": itr})
        wandb.run.log_code()

        for domain_idx, (dataset, data_loader) in enumerate(zip(datasets, data_loaders)):
            print(f"\n=========== Domain {domain_idx} ===========")

            mean_mIoU = 0

            for epoch in range(args.num_domain_epochs):
                print(f"\n--- Epoch {epoch+1}/{args.num_domain_epochs} on Domain {domain_idx} ---")

                wandb.log({"domain": domain_idx, "domain_epoch": epoch})
                wandb.run.log_code()
                ...
                """每个 domain 重新准备 anchor 和 ema_model"""
                anchor = deepcopy(model.state_dict())
                anchor_model = deepcopy(model)
                ema_model = create_ema_model(model)

                outputs = single_gpu_gapatta(
                    args, model, data_loader, args.show, args.show_dir,
                    efficient_test, anchor, ema_model, anchor_model, False, False,
                ...
                if rank == 0:
                    ...
                    if args.eval:
                        results = dataset.evaluate(outputs, args.eval, **kwargs)
                        mIoU = results['mIoU']
                        print(f"✅ [Domain {domain_idx} - Epoch {epoch}] mIoU = {mIoU:.4f}")

                        wandb.log({"mIoU": mIoU})
                        mean_mIoU += mIoU

            mean_mIoU /= args.num_domain_epochs
            All_mIoU += mean_mIoU

            wandb.log({"mean_mIoU_domain_{}".format(domain_idx): mean_mIoU})

        wandb.log({"All_mIoU": All_mIoU / len(datasets)})
    wandb.finish()
